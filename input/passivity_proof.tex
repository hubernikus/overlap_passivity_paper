...
Let us investigate the region where the passivity holds. In this Lemma, it is assumed that all damping values are equal to one, hence we have:
\begin{equation}
...
\begin{figure}[thb]
	...
	\caption{Analysing the system in velocity-space, yields that the system is passive for velocities $\dot{\vecs \xi}$ which are larger than the desired velocity $\vect f(\vecs \xi)$, i.e., outside the dashed orange circle.
