...
\begin{split}
	...
	% & = \frac{\matd{M}^{-1}}{\Gamma^{\mathrm{crit}} - 1} \int \left( (\Gamma - 1) \matd{D}^f + (\Gamma^{\mathrm{crit}} - \Gamma) \matd{D}^{o} \right)  \vecs{\dot \xi} dt \\
	% & = \frac{\matd{M}^{-1}}{\Gamma^{\mathrm{crit}} - 1} \int ( (\| \vecs \xi - \vecs \xi^b \| - 1) \matd{D}^f   \\
	% & \hspace{2cm} + (\Gamma^{\mathrm{crit}} - 1 - \| \vecs \xi - \vecs \xi^b \| \matd{D}^{o} )  \vecs{\dot \xi} dt
	% & = 
	\end{split}
\label{eq:velocity_evolution}
\end{equation}
...
We want to find the position where the agents stops moving towards the obstacle. Hence, we need to observe the behavior along the normal direction towards the obstacle $\vect n(\vecs \xi)$, and see when the velocity along the normal reaches zero.  
\begin{equation}
...
\begin{equation}
	...
	0 & = \left| \vect n(\vecs \xi)^T \, \{ \vecs{\dot \xi} \}_{t} \right| \\
	  ...
	  % & = \frac{\matd{M}}{\hat{\Gamma}^{\mathrm{crit}}} \int ( ( \xi - \xi^b  - 1) \vect n^T \matd{D}^f \vect n \\
	  % & \hspace{2cm} + (\hat{\Gamma}^{\mathrm{crit}} - \xi - \xi^b s^{o} )  {\dot \xi} dt \\
	  % & < \frac{\matd{M}}{\hat{\Gamma}^{\mathrm{crit}}} ((- \xi^b - 1) s^f + (\hat \Gamma^{\mathrm{crit}} - \xi^b) s^o) \xi + 1/2(s^f - s^o) \xi^2 + v^I  \\
  \end{split} 
\end{equation}
