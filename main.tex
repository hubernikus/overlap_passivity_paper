...
However, this method does not consider obstacles when computing $\matd D$. Unfortunately, if a disturbance happens to push the robot into an obstacle that is perpendicular to the direction of motion, it will not be damped. This could lead to the robot crashing into the obstacle. In those cases, we would like to increase the damping of the control and dampen the disturbance.
...
\end{equation}
Away from any obstacle, the behavior is fully controlled by $\vecs {D_{DS}}(\vecs\xi)$, and when approaching one, the damping matrix smoothly shifts to also increase the damping of the conroller in the direction of the obstacle. 
...
The damping matrix, which achieves the dynamical system following $\matd D^{DS}$, is designed as presented in Section~\ref{sec:trad_passive}, following the theory of \cite{kronander2015passive}. $\matd D^{DS}$ has high damping in the direction of the desired velocity $\dot{\vecs \xi}$, but is compliant perpendicular to it.
...
\begin{equation}
  ...
  \end{split}
\end{equation}
where the damping along the nominal direction $s^{DS} \in \mathbb{R}$, obstacle-damping $s^{\mathrm{obs}} \in \mathbb{R}$, and the compliant-damping $s^c \in \mathbb{R}$ are user-defined values which define the behavior of the passive-controller.



\subsubsection{Obstacle Repulsion} \label{sec:obstacle_repulsion}
...
%In the general case, we would have $\lambda_1 = \lambda_{obs}$  corresponding to the damping coefficients associated with moving into an obstacle, $\lambda_2 = \lambda_{DS}$ the tracking damping and $\lambda_3 = \lambda_{perp}$ is the coefficient responsible for being compliant in the remaining direction.
