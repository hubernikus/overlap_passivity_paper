...
\begin{abstract}
Evaluating and updating the obstacle avoidance velocity for an autonomous robot in real-time ensures robustness against noise and disturbances. To obtain the approximated the desired motion with a torque controlled robot, a passive damping controller can be used which remains compliant and ensure a safe response to external perturbations.
Here, we propose a novel approach for designing the passive control policy. Our algorithm has compliant behavior in obstacle-free zones while transitioning to increased damping near obstacles to ensure collision avoidance. This approach ensures stability across diverse scenarios, effectively mitigating disturbances.
